...
export class ProgressieService extends IHeliosService<ProgressieEntity, ProgressieViewEntity> {
    constructor(
        ...
        @InjectRepository(CompetentiesViewEntity) protected readonly competentieViewRepository: Repository<CompetentiesViewEntity>,
    ...
    async getProgressiekaart(filter: ProgressieKaartFilterDTO): Promise<GetObjectsResponse<ProgressiekaartDTO>> {
        const datasetRaw = await this.competentieViewRepository.createQueryBuilder('c')
            .select('c.BLOK as BLOK, c.BLOK_ID as BLOK_ID, c.DOCUMENTATIE as DOCUMENTATIE, p.GELDIG_TOT as GELDIG_TOT, c.ID as ID, p.INGEVOERD as INGEVOERD, p.INSTRUCTEUR_NAAM as INSTRUCTEUR_NAAM, c.LEERFASE as LEERFASE, c.LEERFASE_ID as LEERFASE_ID, c.ONDERWERP as ONDERWERP, p.OPMERKINGEN as OPMERKINGEN, p.ID as PROGRESSIE_ID, p.SCORE as SCORE')
            .leftJoin(qb => {
                return qb
                    .select('*')
