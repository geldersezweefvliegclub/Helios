...
abstract class Helios
{
    ...
    */
    function heeftDataToegang($datum = null, $instructeurs = true): bool
    {
    ...
    */
    function heeftToegangBeheerderDDVW($datum): bool
    {
        $l = MaakObject('Login');
    ...
    */
    function heeftToegangDDWVCrew($datum): bool
    {
        $l = MaakObject('Login');
    ...
    */
    function bestaatTabel($tabel = null): bool
    {
    ...
    */
    function GetSingleObject($conditions = null): ?array
    {
    ...
    */
    function MarkeerAlsVerwijderd($IDs, $verificatie = true): void
    {
    ...
    Het veld VERWIJDERD wordt op "1" gezet.
    */
    function HerstelVerwijderd($IDs): void
    {
    ...
    function Count($query, $conditions = null): int
    {
    ...
    function Elimineer($IDs): void
    {
        Debug(__FILE__, __LINE__, sprintf("Helios.Elimineer('%s')", $IDs));

        if ($this->dbTable == "audit") {
            $this->DbUitvoeren(sprintf("UPDATE `%s` SET `VERWIJDERD`= 1 WHERE ID IN (%s);", $this->dbTable, $IDs));
        } else {
            $this->DbUitvoeren(sprintf("DELETE FROM `%s` WHERE ID IN (%s);", $this->dbTable, $IDs));

            $audit = MaakObject("Audit");
            $audit->AddObject($this->dbTable, $this->Naam, "Elimineer", $IDs, null, null, null);
        }
    }

    // Functie voor slim laden van datastores in web applicatie
    function LaatsteAanpassing($query, $conditions = null, $veld = null): string
    {
    ...
    /// aangepast wordt.
    ///-------------------------------------------------------------------------------------------------------------------------------------
    function DbToevoegen($array): int
    {
    ...
    function DbOpvraag($query, $params = null): ?array
    {
        global $db;
