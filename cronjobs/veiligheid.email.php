...
<p>
    Beste veiligheidsmanager,
</p>
<p>
    In onze startadministratie staan de volgende incidenten genoteerd in het dagrapport van <b>%s</b>
</p>
...
else
{
    ...
    foreach ($auditRecords['dataset'] as $record)
    {
        ...
        if ($voor['INCIDENTEN'] == $resultaat['INCIDENTEN']) {
        ...
        if (!in_array($resultaat['DATUM'], $datumArray)) {
            ...
            $body = sprintf($htmlContent, $datumString, $resultaat['INCIDENTEN']);
            ...
            $mail->addAddress($veiligheidsmanager['EMAIL'], $veiligheidsmanager['NAAM']); 
